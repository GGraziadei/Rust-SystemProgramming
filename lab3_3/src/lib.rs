...
impl MatchResult<'_, '_ >{
    ...
    //Sposto il tempo di vita del nodo in dietro per evitare che ci siano sovrapposizioni
    pub fn check_rc<'a, 'b> (queries: &'a [&str], node: &'b mut Node ) -> Option<Vec<&'a &'a str>>
    {
        let timestamp = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as u64;
        let mut result = vec![];
        ...
        if node.is_file_mut() {
            ...
            for query in queries.iter() {
                ...
                if expression[0].eq("name"){
                    ...
                    if file.type_ == FileType::Text
                    {
                        ...
                        if content.contains(&expression[1].to_string()){
                            result.push(query);
                        }
                    }
                }
                else if expression[0].eq("larger"){
                    if file.content.len() >= expression[1].parse().unwrap(){
                        result.push(query);
                    }
                }
                else if expression[0].eq("smaller"){
            ...
            for query in queries.iter() {
                ...
                if expression[0].eq("name"){
                    if expression[1].eq(&dir.name){
                        result.push(query );
                    }
                } else if expression[0].eq("newer"){
                    if dir.creation_time >= timestamp {
                        result.push(query);
                    }
                }
                else if expression[0].eq("older"){
...
impl fmt::Display for File{
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "(Name: {}, creation_time {})", self.name, self.creation_time)
    }
}
...
impl fmt::Debug for File{
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "(Name: {}, creation_time {}, type {})", self.name, self.creation_time, self.type_ )
    }
}
...
impl Dir{
    pub fn new(path: &str) -> Self {
        ...
        Self{
            name: path.to_string(),
            creation_time,
            children : RefCell::<Vec<Node>>::new(children)
        }
    }
...
impl  FileSystem {
        ...
        for step in Self::get_path(&path_name).rev() {
            ...
            if step.ne(&curr_dir.name) {
                match Dir::get_index_by_name(&curr_dir, &step.to_string()) {
                    None => { panic!() }
                    Some(index) => {
                        curr_dir = Node::get_mut_dir(&mut curr_dir.children.get_mut()[index]).unwrap();
                    }
                };
            }
        }
        curr_dir.children.get_mut().push(Node::Dir(created_dir));
        path_name
    }
    ...
    pub fn rm_dir<'a>(&'a mut self, path_name: &'a str) -> &str {
        ...
        */
        ...
        match Dir::get_index_by_name(&curr_dir, &path_name.to_string()) {
            None => { panic!() }
            Some(index) => {
                curr_dir.children.get_mut().remove(index)
            }
        };
        path_name
    }
    ...
    pub fn new_file<'a>(&'a mut self, path_name: &'a str, file: File) -> &str {
        ...
        curr_dir.children.get_mut().push(Node::File(file));
        path_name
    }
    ...
    pub fn get_file<'a>(&'a mut self, path_name: &'a str) -> Option<&mut File> {
        ...
        match Dir::get_index_by_name(&curr_dir, &path_name.to_string()) {
            None => { return None; }
            Some(index) => {
                return Node::get_mut_file(&mut curr_dir.children.get_mut()[index]);
            }
        };
    }
    ...
    pub fn search<'a >(&'a mut self, queries: &'a [&str]) -> Option<MatchResult> {
        ...
        while dir_queue.len() > 0 {
            ...
            for node in dir.children.get_mut().iter_mut(){

                //Node as a mut reference to a Node
                let result = MatchResult::check_rc(&queries,node);
                if result.is_some() {
